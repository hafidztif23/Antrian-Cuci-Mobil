...
// Fungsi untuk mencari dan menampilkan mobil berdasarkan plat nomor
void searchCar(char *plate) {
    ...
    for (int i = 0; i < MAX_STATION; i++) {
        if (washingStations[i].processing != NULL && strcmp(washingStations[i].processing->plate, plate) == 0) {
            ...
            time_t processedTime = mktime(gmtime(&now)) + (washingStations[i].processing->washingTime + washingStations[i].processing->dryingTime) * 60;
    ...
    if (!found) {
        for (int i = 0; i < MAX_STATION; i++) {
            if (dryingStations[i].processing != NULL && strcmp(dryingStations[i].processing->plate, plate) == 0) {
                ...
                // Kalkulasi waktu selesai pencucian berdasarkan waktu mulai dan waktu pengerjaan
                time_t processedTime = mktime(gmtime(&now)) + (dryingStations[i].processing->washingTime + dryingStations[i].processing->dryingTime) * 60;
                printf("Waktu selesai pencucian : %s\n", ctime(&processedTime));
                break;
            }
        }
    ...
    if (!found) {
        ...
        while (current != NULL) {
            if (strcmp(current->plate, plate) == 0) {
                ...
                // Belum diproses, jadi belum ada waktu mulai pencucian dan selesai pencucian
